...
//For loop placed to test if multiple fq files are processed in the same go. grh_chunk, grh_ret, grh_fn are variables used for testing
	...
	for (grh_iter=0;grh_iter<grh_chunk;grh_iter++)
	{
		...
		while(fdram == NULL) {
			...
			sleep(2);
			fdram = fopen(argv[optind + 1], "rb");// LOADING THE READ FILE after waiting
		...
		kt_pipeline(no_mt_io? 1 : 2, process, &aux, 3); // pipeline function to call the process function.
		grh_ret = err_gzclose(fp);
		if(grh_ret<0){
						perror("gz close has failed\n");	
					}
					
		grh_ret = kclose(ko);	
		if(grh_ret<0){
						perror("kclose has failed\n");	
					}
					
					
		grh_ret=remove(argv[optind + 1]);//to remove the previous fqfile from RAM
		if(grh_ret<0){
						perror("System command has failed\n");	
					}
	}
